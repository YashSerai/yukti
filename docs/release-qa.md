...
- 2026-08-02, production version 28, commit `15c600c`: signed-out desktop and phone landing, owner Today, passport task, People, Purchases, Activity, and Calendar were inspected at 1440 x 900 and 390 x 844. The device-width viewport fix removed mobile horizontal overflow; the landing headline settles into three lines; stale checkout records and generated reminder shells are absent; Sarah's flower rule is active for August 28; Calendar import refreshed at 3:17 AM; and browser error logs were empty. Invalidated by app, CSS, migration, deployment, or production-data changes.
- 2026-08-02, production version 29, commit `3ee0967`: owner Calendar showed the daily/manual refresh contract and `Refresh now`; the attended refresh advanced `Last checked` from 3:17 AM to 1:05 PM. Browser logs contained no Yukti application errors and the 1905 px viewport had no horizontal overflow. Scheduler dry-run `30764879226` passed with Calendar due `0` immediately after that refresh and reminder due `0`. No new or changed Calendar event was available, so a freshly generated event note was not visually certified. Invalidated by Calendar sync, task enrichment, scheduler, migration, UI, or deployment changes.
